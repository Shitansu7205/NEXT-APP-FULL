...
const Login = (prop) => {
    ...
    const getSubmit = async (e) => {
        e.preventDefault(); // No need to await this
        const formData = {
            mail,
            password
        }
        ...
        try {
            ...
            if (response.ok) {
                ...
                const tokendata = data.token;       // retrive and store the token data from the api res.
    ...
    return (
        <>
            <form className="max-w-sm mx-auto mt-32 p-4 bg-white rounded-lg shadow-md dark:bg-gray-800 form-signin" onSubmit={getSubmit} >
                ...
                <p className="text-sm font-normal text-gray-500 dark:text-gray-400 text-center">Fill the form to login</p>
